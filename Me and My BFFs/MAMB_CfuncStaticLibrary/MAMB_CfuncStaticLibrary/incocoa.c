/* incocoa.c  */
/* simulates inside cocoa App, calling mambutil.c routines */
...
void get_mbrs_from(char *grpmbr_file, int *num_in_grp)
{
  ...
  *num_in_grp = 0;
  ...
  while (fgets(linebuf2, 128, FP_grp_members) != NULL) {
    scharout(linebuf2, '\n');
    mamb_csv_put(linebuf2, strlen(linebuf2));
    *num_in_grp = *num_in_grp + 1;
  }
...
/*   char pad_spaces[32]; */
  ...
  */
  for (longest_A=0,longest_B=0, i=0; i <= out_rank_idx; i++) {
    ...
    lenA = strlen(out_rank_lines[i]->person_A);
    lenB = strlen(out_rank_lines[i]->person_B);
...
/*   char pad_spaces[32]; */
  ...
  */
  for (longest_A=0,i=0; i <= out_trait_idx; i++) {
    ...
    lenA = strlen(out_trait_lines[i]->person_name);
    if(lenA > longest_A) longest_A = lenA;
  }
